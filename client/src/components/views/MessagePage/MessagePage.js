...
function Message() {
  ...
  useEffect(() => {
    ...
    axios
      .post("/api/message/getMessage", { messageTo: user.userData._id })
      .then((res) => {
        if (res.data.success) {
      ...
      {Messages ? (
        <Styled.MessageTable className="messageTable">
          <thead>
            <tr>
              <th>
                <Checkbox onChange={setCheckAll}></Checkbox>
              </th>
              <th>보낸사람</th>
              <th>내용</th>
              <th>날짜</th>
                ...
                <tr key={idx} className="msg">
                  <td className="msgCheck">
                    <Checkbox
                      ...
                      checked={
                        SeletedMsg.indexOf(data._id) !== -1 ? true : false
                      }
                    />
                  </td>
                  <td className="msgFrom">
                    <Writer
                      writer={data.messageFrom._id}
                      name={data.messageFrom.name}
                    />
                  </td>
