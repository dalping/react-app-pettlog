...
function InputReplyComment(props) {
  ...
  const onSubmitReplyComment = (e) => {
    ...
    axios.post("/api/comment/uploadReplyComment", variable).then((res) => {
      if (res.data.success) {
        setInputReply("");
    ...
    <div className="inputReplyComment">
      ...
      >
        <Input
          type="text"
          value={InputReply}
          onChange={InputReplyHandelr}
          placeholder={`@${props.comment.userId.name} 에게 댓글 달기...`}
        />
        {InputReply.length > 0 ? (
          <Button type="text" onClick={onSubmitReplyComment}>
            작성
          </Button>
        ) : (
